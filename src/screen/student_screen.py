...
def student_dashboard():
    ...
    # Create a single column for buttons
    with st.container():
        ...
        # Refresh button directly below Enroll button
        col_refresh, col_empty = st.columns([1, 3])
        with col_refresh:
            if st.button('🔄 Refresh Attendance', type='secondary', width='stretch', help='Refresh attendance information'):
                st.rerun()
...
def student_screen():
    ...
    if photo_source:
        ...
        with st.spinner('AI is scanning..'):
            # Direct embedding comparison for face login
            ...
            if not face_encodings:
                ...
                if not all_students:
                    st.info('No registered students found. Please register first.')
                    ...
                    for student in all_students:
                        ...
                        if stored_embedding:
                            similarity = np.dot(captured_embedding, np.array(stored_embedding))
                    ...
                    if best_match and best_similarity >= threshold:
                        ...
                        st.toast(f'Welcome Back {best_match["name"]}!')
                        time.sleep(1)
                        st.rerun()
                    else:
